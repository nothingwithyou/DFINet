...
@torch.no_grad()
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, masks, filename):
    N, C, H, W = x_src.size()
    s_ref = nets.style_encoder(x_ref, masks, y_ref)
    masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None
    x_fake = nets.generator(x_src, s_ref, masks=masks)
    s_src = nets.style_encoder(x_src, masks, y_src)
...
@torch.no_grad()
def translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, mask, filename):
    N, C, H, W = x_src.size()
    latent_dim = z_trg_list[0].size(1)
    devices = x_src.device
    x_mask = x_src * (1 - mask)
    x_concat = [x_src, x_mask]
    # masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None

    for i, y_trg in enumerate(y_trg_list):
        z_many = torch.randn(10000, latent_dim).to(devices)
        y_many = torch.LongTensor(10000).to(devices).fill_(y_trg[0])
        ...
        for z_trg in z_trg_list:
            ...
            first_out, _, x_fake = nets.generator(x_mask, s_trg, masks=mask)
            x_concat += [x_fake]
...
@torch.no_grad()
def translate_using_reference(nets, args, x_src, x_ref, y_ref, mask, filename):
    N, C, H, W = x_src.size()
    x_mask = x_src * (1 - mask)
    mask_1 = torch.cat((mask, mask, mask), 1)
    mask_1 = mask_1 * 2 - 1
    x_src_with_wb = torch.cat([x_src, mask_1, x_mask], dim=0)
    devices = x_src.device
    # masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None
    s_ref = nets.style_encoder(x_ref, mask, y_ref)
    s_ref_list = s_ref.unsqueeze(1).repeat(1, N, 1)
    x_concat = [x_src_with_wb]
    for i, s_ref in enumerate(s_ref_list):
        first_out, _, x_fake = nets.generator(x_mask, s_ref, masks=mask)
        first_out = first_out.cpu().argmax(1)
        out_parsing_color = torch.zeros((N, 3, H, W)) + 255
        for pi in range(1, 20):
            index = np.where(first_out == pi)
            out_parsing_color[index[0], :, index[1], index[2]] = part_colors[pi]
        out_parsing_color = out_parsing_color / 255 * 2 - 1
        x_fake_with_ref = torch.cat([x_ref[i:i+1], out_parsing_color.to(devices), x_fake], dim=0)
...
@torch.no_grad()
def save_sample_png(nets, args, inputs, step, pixel_max_cnt=255):
    # Save image one-by-one
    x_src, x_p, y_org, mask = inputs.x, inputs.p, inputs.y, inputs.mask
    filename = ospj(args.sample_dir, '%06d_pre.jpg' % (step))
    N, C, H, W = x_src.size()
    x_mask = x_src * (1 - mask)
    # s_trg = None
    s_trg = nets.style_encoder(x_mask, mask, y_org)
    first_out, _, x_fake = nets.generator(x_mask, s_trg, masks=mask)
    # first_out, _, x_fake = nets.generator(x_mask, None, masks=mask)
    first_out = first_out.cpu().argmax(1)
    ...
    for pi in range(1, 20):
        ...
        out_parsing_color[index[0], :, index[1], index[2]] = part_colors[pi]
    out_parsing_color = out_parsing_color / 255 * 2 - 1
    x_concat = [x_src, mask, x_mask, out_parsing_color.to(x_src.device), x_fake]
    x_concat = torch.cat(x_concat, dim=0)
    save_image(x_concat, N, filename)
    del x_concat
...
def debug_image(nets, args, inputs, step):
    x_src, x_p, y_src, mask = inputs.x_src, inputs.p_src, inputs.y_src, inputs.mask
    ...
    # translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, mask, filename)
    ...
    for psi in [0.5, 0.7, 1.0]:
        ...
        translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, mask, filename)

    # reference-guided image synthesis
    filename = ospj(args.sample_dir, '%06d_reference.jpg' % (step))
    translate_using_reference(nets, args, x_src, x_ref, y_ref, mask, filename)


@torch.no_grad()
def lat_image(nets, args, inputs, step):
    x_src, x_p, y_src, mask = inputs.x, inputs.p, inputs.y, inputs.mask
    device = x_src.device
    N = x_src.size(0)
...
@torch.no_grad()
def video_ref(nets, args, x_src, x_ref, y_ref, mask, fname):
    video = []
    s_ref = nets.style_encoder(x_ref, mask, y_ref)
    s_prev = None
