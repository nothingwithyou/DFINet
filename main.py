    ...
    if args.mode == 'train':
        ...
        loaders = Munch(src=get_train_loader(root=args.train_img_dir,
                        ...
                        val=get_test_loader(root=args.val_img_dir,
                                            ...
                                            mask_type=args.mask,
                                            num_workers=args.num_workers))
        solver.train_(loaders)

    if args.mode == 'pre_train':
        solver = Solver(args, pre=True)
        assert len(subdirs(args.train_img_dir)) == args.num_domains
        assert len(subdirs(args.val_img_dir)) == args.num_domains
        pre_loaders = Munch(src=get_train_loader(root=args.train_img_dir,
                            ...
                            val=get_test_loader(root=args.val_img_dir,
                                                ...
                                                mask_type=args.mask,
                                                num_workers=args.num_workers))
        solver.pre_train(pre_loaders)

    elif args.mode == "train_1":
        solver = Solver(args)
        assert len(subdirs(args.train_img_dir)) == args.num_domains
        assert len(subdirs(args.val_img_dir)) == args.num_domains
        pre_loaders = Munch(src=get_train_loader(root=args.train_img_dir,
        ...
        loaders = Munch(src=get_test_loader(root=args.val_img_dir,
                                            img_size=args.img_size,
                                            batch_size=args.val_batch_size,
                                            shuffle=True,
                                            from_dir=args.from_mdir,
...
def make_args():
    ...
    parser.add_argument('--lambda_cyc', type=float, default=4,
                        help='Weight for cyclic consistency loss')
    parser.add_argument('--lambda_sty', type=float, default=1,
                        help='Weight for style reconstruction loss')
    parser.add_argument('--lambda_ds', type=float, default=2,
                        help='Weight for diversity sensitive loss')
    parser.add_argument('--lambda_pl1', type=float, default=0.01, help='the parameter of parsing L1Loss')
    parser.add_argument('--lambda_il1', type=float, default=10, help='the parameter of image L1Loss')
    ...
    parser.add_argument('--s_loss', type=float, default=10, help='STYLE_LOSS_WEIGHT')
    ...
    parser.add_argument('--mode', type=str, required=True,
                        choices=['train', 'eval', 'sample', 'pre_train', 'train_1'],
    ...
    parser.add_argument('--logs_dir', type=str, default='runs/experiment_2',
                        help='Directory for saving meters logs in tensorboard')
    ...
    parser.add_argument('--sample_every', type=int, default=2000)  # 5000
    parser.add_argument('--save_every', type=int, default=4000)  # 10000
