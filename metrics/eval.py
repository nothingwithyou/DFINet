...
def calculate_metrics(nets, args, step, mode):
    ...
    for trg_idx, trg_domain in enumerate(domains):
        ...
        for src_idx, src_domain in enumerate(src_domains):
            ...
            for i, inputs in enumerate(tqdm(loader_src, total=len(loader_src))):
                x_src, parsing, label, mask = inputs
                N = x_src.size(0)
                ...
                for j in range(args.num_outs_per_domain):
                    if mode == 'latent':
                        ...
                        try:
                            x_refs = next(iter_ref).to(device)
                        except:
                            iter_ref = iter(loader_ref)
                            x_refs = next(iter_ref).to(device)
                        x_ref, parsing, label, mask = x_refs
                        if x_ref.size(0) > N:
                            x_ref = x_ref[:N]
                        s_trg = nets.style_encoder(x_ref, y_trg)
                    x_mask = x_src * (1 - mask)
                    _, _, x_fake = nets.generator(x_mask, s_trg, masks=mask)
